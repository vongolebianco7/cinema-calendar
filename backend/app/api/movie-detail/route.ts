...
export async function GET(request:Request){const limited=rateLimited(request,60);if(limited)return limited;
 ...
 try{
  ...
  let related:any[]=[];try{const ru=new URL(BASE+`/movie/${id}/recommendations`);ru.searchParams.set("language","ja-JP");const rh=auth(ru);if(rh){const rr=await fetch(ru,{headers:rh,next:{revalidate:86400}});if(rr.ok){const rd:any=await rr.json();related=(rd.results||[]).slice(0,12).map((x:any)=>({id:x.id,tmdbId:x.id,title:x.title||x.original_title,overview:x.overview||"",year:(x.release_date||"").slice(0,4),poster:x.poster_path?`https://image.tmdb.org/t/p/w342${x.poster_path}`:null,score:x.vote_average,genre_ids:Array.isArray(x.genre_ids)?x.genre_ids:[]}))}}}catch(e){console.warn("recommendations unavailable",e)}
