...
export async function GET(request:Request){
 ...
 try{
  ...
  const director=crew.find((x:any)=>x.job==="Director")?.name||"";
  ...
  return json({movie:{id:m.id,tmdbId:m.id,title:m.title,original_title:m.original_title,overview:m.overview,date:m.release_date,year:(m.release_date||"").slice(0,4),runtime:m.runtime||null,genres:(m.genres||[]).map((x:any)=>x.name),countries:(m.production_countries||[]).map((x:any)=>x.name),director,cast:cast.slice(0,12).map((x:any)=>x.name),score:m.vote_average,votes:m.vote_count,poster:m.poster_path?`https://image.tmdb.org/t/p/w500${m.poster_path}`:null,backdrop:m.backdrop_path?`https://image.tmdb.org/t/p/w780${m.backdrop_path}`:null,tmdb:`https://www.themoviedb.org/movie/${m.id}`,trailer_url:trailer?`https://www.youtube.com/watch?v=${trailer.key}`:null,event:"catalog",source:"tmdb",catalog:true,availability:{flatrate:providers(jp.flatrate),rent:providers(jp.rent),buy:providers(jp.buy),source_url:jp.link||null}}}});
